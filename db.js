...
async function insertNewAuthor(db, author) {
  ...
  console.log(`Author ${author.firstName} inserted with id: ${result.insertedId}`);
}



async function createBooksIndexses(db) {
  const IndexByNumPages = await db.collection('books').createIndex({ numPages: 1 });
  const IndexByAuthor = await db.collection('books').createIndex({ authorId: 1 });
...
async function getNumPagesMoreThen250(db) {
  const filteredDocs = await db.collection('books').find(({ numPages: { $gt: 250 } })).toArray();
